...
# Function: \entity\hostile\jevil\attack\bomb\base\loop.mcfunction

# Runs every tick off entities tagged "jevil_is_attacking_bomb"
...
# Summon attack_indicator_bomb based on variant
execute if entity @s[scores={afro.math_0=0}] run function theafroofdoom:entity/hostile/jevil/attack/bomb/base/condition/loop/afro.math_0/0

# Terminate at {attack_clock=attack_length} based on variant
execute if score @s attack_clock = @s attack_length run function theafroofdoom:entity/hostile/jevil/attack/bomb/base/condition/loop/attack_clock/attack_length
