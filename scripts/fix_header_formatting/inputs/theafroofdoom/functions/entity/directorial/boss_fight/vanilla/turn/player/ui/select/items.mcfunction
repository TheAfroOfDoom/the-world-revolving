...
playsound theafroofdoom:player.do.item.heal player @a ~ ~ ~ 2 1 0.1

# TODO(jordan): Come back here after some beta tests to determine if we want to revive dead players using clubs sandwich / top cake
...
# Clubs sandwich
execute if score @s ate_cookie matches 1.. run scoreboard players add @a[team=player] hearts_needed 4

# Lancer cookie / Rouxls roux
execute if score @s ate_potato matches 1.. run scoreboard players add @s hearts_needed 7

# Dark candy
execute if score @s ate_trop_fish matches 1.. run scoreboard players add @s hearts_needed 5
